...
class Dumper
{
	...
	public static function toHtml($var, array $options = NULL)
	{
		...
		return '<pre class="tmdebug-dump"'
			. ($file ? Helpers::createHtml(' title="%in file % on line %" data-tmdebug-href="%"', "$code\n", $file, $line, Helpers::editorUri($file, $line)) . '>' : '>')
	...
	{
		return htmlspecialchars_decode(strip_tags(preg_replace_callback('#<span class="tmdebug-dump-(\w+)">|</span>#', function($m) {
	...
	{
		return "<span class=\"tmdebug-dump-null\">NULL</span>\n";
	}


	private static function dumpBoolean(& $var)
	{
		return '<span class="tmdebug-dump-bool">' . ($var ? 'TRUE' : 'FALSE') . "</span>\n";
	}


	private static function dumpInteger(& $var)
	{
		return "<span class=\"tmdebug-dump-number\">$var</span>\n";
	}
	...
	{
		return '<span class="tmdebug-dump-string">"'
	...
	private static function dumpArray(& $var, $options, $level)
	{
		...
		$out = '<span class="tmdebug-dump-array">array</span> (';
		...
		if (empty($var)) {
			...
			$out = '<span class="tmdebug-toggle' . ($collapsed ? ' tmdebug-collapsed' : '') . '">'
				. $out . count($var) . ")</span>\n<div" . ($collapsed ? ' class="tmdebug-collapsed"' : '') . '>';
			$var[$marker] = TRUE;
			foreach ($var as $k => & $v) {
				if ($k !== $marker) {
					$k = preg_match('#^\w{1,50}\z#', $k) ? $k : '"' . htmlspecialchars(self::encodeString($k, $options[self::TRUNCATE])) . '"';
					$out .= '<span class="tmdebug-dump-indent">   ' . str_repeat('|  ', $level) . '</span>'
						. '<span class="tmdebug-dump-key">' . $k . '</span> => '
	...
	private static function dumpObject(& $var, $options, $level)
	{
		...
		$out = '<span class="tmdebug-dump-object"'
			. ($options[self::LOCATION] && ($editor = Helpers::editorUri($rc->getFileName(), $rc->getStartLine())) ? ' data-tmdebug-href="' . htmlspecialchars($editor) . '"' : '')
			. '>' . get_class($var) . '</span> <span class="tmdebug-dump-hash">#' . substr(md5(spl_object_hash($var)), 0, 4) . '</span>';
		...
		if (empty($fields)) {
			...
			foreach ($fields as $k => & $v) {
				$vis = '';
				if ($k[0] === "\x00") {
					$vis = ' <span class="tmdebug-dump-visibility">' . ($k[1] === '*' ? 'protected' : 'private') . '</span>';
					$k = substr($k, strrpos($k, "\x00") + 1);
				}
				$k = preg_match('#^\w{1,50}\z#', $k) ? $k : '"' . htmlspecialchars(self::encodeString($k, $options[self::TRUNCATE])) . '"';
				$out .= '<span class="tmdebug-dump-indent">   ' . str_repeat('|  ', $level) . '</span>'
					. '<span class="tmdebug-dump-key">' . $k . "</span>$vis => "
					. self::dumpVar($v, $options, $level + 1);
			}
			array_pop($list);
			return $out . '</div>';
	...
	private static function dumpResource(& $var, $options, $level)
	{
		...
		$out = '<span class="tmdebug-dump-resource">' . htmlSpecialChars($type) . ' resource</span> '
			. '<span class="tmdebug-dump-hash">#' . intval($var) . '</span>';
		if (isset(self::$resources[$type])) {
			$out = "<span class=\"tmdebug-toggle tmdebug-collapsed\">$out</span>\n<div class=\"tmdebug-collapsed\">";
			foreach (call_user_func(self::$resources[$type], $var) as $k => $v) {
				$out .= '<span class="tmdebug-dump-indent">   ' . str_repeat('|  ', $level) . '</span>'
					. '<span class="tmdebug-dump-key">' . htmlSpecialChars($k) . "</span> => " . self::dumpVar($v, $options, $level + 1);
			}
			return $out . '</div>';
		}
		return "$out\n";
		...
		foreach (debug_backtrace(PHP_VERSION_ID >= 50306 ? DEBUG_BACKTRACE_IGNORE_ARGS : FALSE) as $item) {
			if (isset($item['class']) && $item['class'] === __CLASS__) {
				...
				try {
					...
					if (preg_match('#\s@TmDebugSkipLocation\s#', $reflection->getDocComment())) {
						$location = $item;
						continue;
					}
