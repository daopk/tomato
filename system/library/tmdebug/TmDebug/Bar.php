		...
		foreach ($this->panels as $id => $panel) {
			...
			try {
				...
				if ($tab && $panel instanceof \Nette\Diagnostics\IBarPanel) {
					$panelHtml = preg_replace('~(["\'.\s#])nette-(debug|inner|collapsed|toggle|toggle-collapsed)(["\'\s])~', '$1tmdebug-$2$3', $panelHtml);
					$panelHtml = str_replace('tmdebug-toggle-collapsed', 'tmdebug-toggle tmdebug-collapsed', $panelHtml);
				}
				...
				$panels[] = array(
					'id' => "error-$idHtml",
					'tab' => "Error in $id",
					'panel' => '<h1>Error: ' . $id . '</h1><div class="tmdebug-inner">'
