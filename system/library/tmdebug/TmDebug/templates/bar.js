...
(function(){
	...
	var Panel = TmDebug.DebugPanel = function(id) {
		this.id = 'tmdebug-debug-panel-' + id;
		this.elem = $('#' + this.id);
	};

	Panel.PEEK = 'tmdebug-mode-peek';
	Panel.FLOAT = 'tmdebug-mode-float';
	Panel.WINDOW = 'tmdebug-mode-window';
	Panel.FOCUSED = 'tmdebug-focused';
	Panel.zIndex = 20000;
	...
	Panel.prototype.init = function() {
		...
		this.elem.find('.tmdebug-icons').find('a').bind('click', function(e) {
			if (this.rel === 'close') {
				_this.toPeek();
			} else {
				_this.toWindow();
	...
	Panel.prototype.toWindow = function() {
		...
		doc.write('<!DOCTYPE html><meta charset="utf-8"><style>' + $('#tmdebug-debug-style').dom().innerHTML + '<\/style><script>' + $('#tmdebug-debug-script').dom().innerHTML + '<\/script><body id="tmdebug-debug">');
		doc.body.innerHTML = '<div class="tmdebug-panel tmdebug-mode-window" id="' + this.id + '">' + this.elem.dom().innerHTML + '<\/div>';
		var winPanel = win.TmDebug.Debug.getPanel(this.id);
		win.TmDebug.Dumper.init();
	...
	var Bar = TmDebug.DebugBar = function() {
	};

	Bar.prototype.id = 'tmdebug-debug-bar';
	...
	Bar.prototype.init = function() {
		...
		elem.find('a').bind('click', function(e) {
			...
			if (this.rel && this.rel !== 'close' && !elem.hasClass('tmdebug-dragged')) {
	...
	var Debug = TmDebug.Debug = {};

	Debug.init = function() {
		Debug.initResize();
		(new Bar).init();
		$('.tmdebug-panel').each(function() {
			Debug.getPanel(this.id).init();
		});
	};

	Debug.getPanel = function(id) {
		return new Panel(id.replace('tmdebug-debug-panel-', ''));
	};
