...
class Debugger
{
	...
	/** @deprecated */
	public static $mailer = array('TmDebug\Logger', 'defaultMailer');
	...
	public static function enable($mode = NULL, $logDirectory = NULL, $email = NULL)
	{
		...
		if (!self::$enabled) {
			...
			foreach (array('TmDebug\Bar', 'TmDebug\BlueScreen', 'TmDebug\DefaultBarPanel', 'TmDebug\Dumper',
				'TmDebug\FireLogger', 'TmDebug\Helpers', 'TmDebug\Logger', ) as $class) {
				class_exists($class);
			}
		...
		if (self::$productionMode) {
			...
			try {
				...
				if ($file && !headers_sent()) {
					header("X-TmDebug-Error-Log: $file");
				}
				echo "$exception\n" . ($file ? "(stored in $file)\n" : '');
	...
	/**
	 * Dumps information about a variable in TmDebug Debug Bar.
	 * @TmDebugSkipLocation
	 * @param  mixed  variable to dump
